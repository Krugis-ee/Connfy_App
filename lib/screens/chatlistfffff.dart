...
class ChatListffff extends StatefulWidget {
  final authcode;

  ChatListffff(this.authcode);

  _MyAppState createState() => _MyAppState(this.authcode);
}

class _MyAppState extends State<ChatListffff> {
  var authcode;
  ...
  Future<void> _refreshList() async {
    try {
      List<dynamic> newActiveUsersList = await _fetchListItemsnew(UserId, connected_wifi);
      setState(() {
  ...
  _fetchListItems(dynamic userid, String ssid) async {
    ...
    var request = http.Request('POST', Uri.parse('https://connfy.ragutis.com/api/multi_api'));
  ...
  Future<void> _initNetworkInfo() async {
    ...
    if (wifiName != null && wifiName.isNotEmpty) {
      ...
      List newActiveUsersList = await _fetchListItemsnew(UserId, "empty");
      setState(() {
        activeUsersList = newActiveUsersList;
      });
  ...
  createChatroomAndStartConversation(String uid, String username, String userid, dynamic imageurl) {
    ...
    if (uid != Constants.myName) {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context) => Chat(chatRoomId, uid, username, "1", userid, imageurl, Constants.myName, "")));
    } else {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
            ...
            bottom: PreferredSize(child: getAppBottomView(), preferredSize: const Size.fromHeight(40.0)),
          ),
          body: wifi_sttus == 1
              ? Container(
                              ...
                              itemBuilder: (ctx, index) {
                                return activeUsersList[index]['chat_id'] == Constants.myName
                                    ...
                                    : GestureDetector(
                                        ...
                                        onTap: () {
                                          print(activeUsersList[index]['id']);
...
/*  Future<void> _fetchListItems(dynamic userid) async {
  ...
  Future<void> _updateUserStatus(int status) async {
    var headers = {'Content-Type': 'application/json'};
    var request = http.Request('POST', Uri.parse('https://connfy.ragutis.com/api/update_status'));
