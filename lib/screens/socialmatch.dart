...
class _MyAppState extends State<SocilamatchList> {
 ...
 /* Future<void> _updateUserStatus(int status) async {
    ...
    if (history_id == null || history_id.toString().isEmpty || history_id.toString() == 'null' || history_id.toString().length == 0) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (responseData.containsKey('data')) {
            ...
            var shopid = wifiDetails['shop_id'];
...
*/
  ...
  Future retrive() async {
    ...
    // Call callUpdate only if historyId is null or empty and shopidn is not empty
    /*if (historyId.toString().isEmpty && shopidn.toString().isNotEmpty) {
      if (historyId.toString().isEmpty && shopidn.toString().isNotEmpty) {
        await showLoader(context); // Show loader
  ...
  createChatroomAndStartConversation(String uid, String username, String userid, dynamic imageurl, String token) {
    ...
    if (uid != Constants.myName) {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context) => Chat(chatRoomId, uid, username, "2", userid, imageurl, Constants.myName, token)));
    } else {
