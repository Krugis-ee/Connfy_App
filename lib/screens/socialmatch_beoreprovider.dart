...
class SocilamatchListoldd extends StatefulWidget {
  final authcode;

  SocilamatchListoldd(this.authcode);

  _MyAppState createState() => _MyAppState(this.authcode);
}

class _MyAppState extends State<SocilamatchListoldd> {
  var authcode;
  ...
  Future<void> _initNetworkInfo() async {
    ...
    if (wifiName != null && wifiName.isNotEmpty) {
      ...
      if (config_wifi == cleanedSsid) {
        List newActiveUsersList = await _fetchListItemsnew(UserId, cleanedSsid);
        setState(() {
          activeUsersList = newActiveUsersList;
          activeUsersList.sort((a, b) {
            DateTime timeA = a['chat_time'] == null || a['chat_time'] == 'null' ? DateTime.fromMillisecondsSinceEpoch(0) : DateFormat("yyyy-MM-dd HH:mm:ss").parse(a['chat_time']);
