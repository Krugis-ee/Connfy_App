...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future setAuthValue() async {
    ...
    if (status) {
      ...
      if (email.isNotEmpty) {
        ...
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => ThankYouPage(uemail, UserId)));
      }
    } else {
      if (email.isEmpty) {
  ...
  startTime() async {
    ...
    if (firstTime != null && !firstTime) {
      // Not first time
      return new Timer(_duration, getLoggedInState);
    } else {
      // First time
      prefs.setBool('first_time', false);
      return new Timer(_duration, navigateUser1);
    }
  }
